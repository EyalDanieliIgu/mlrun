...
def test_build_kv_cursor_filter_expression():
    ...
    store_type_object = mlrun.api.crud.model_monitoring.model_endpoint_stores.ModelEndpointStoreType(value="kv")

    endpoint_target = store_type_object.to_endpoint_target(project=TEST_PROJECT, access_key=V3IO_ACCESS_KEY)
...
def test_sql_target_list_model_endpoints():
    ...
    store_type_object = mlrun.api.crud.model_monitoring.model_endpoint_stores.ModelEndpointStoreType(value="sql")
    endpoint_target = store_type_object.to_endpoint_target(project=TEST_PROJECT, connection_string=CONNECTION_STRING)
