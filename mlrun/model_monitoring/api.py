...
def _model_endpoint_validations(model_endpoint: ModelEndpoint,model_path: str = "",
                                sample_set_statistics: typing.Dict[str, typing.Any] = None,
                                drift_threshold: float = 0.7,
                                possible_drift_threshold: float = 0.5
                                ):
    ...
    current_drift_threshold = model_endpoint.spec.monitor_configuration.get(
        EventFieldType.DRIFT_DETECTED_THRESHOLD, mlrun.mlconf.model_endpoint_monitoring.drift_thresholds.default.drift_detected)
    current_possible_drift_threshold = model_endpoint.spec.monitor_configuration.get(
        EventFieldType.POSSIBLE_DRIFT_THRESHOLD, mlrun.mlconf.model_endpoint_monitoring.drift_thresholds.default.possible_drift
    )
    ...
    if possible_drift_threshold and current_possible_drift_threshold != possible_drift_threshold:
        raise mlrun.errors.MLRunInvalidArgumentError(
